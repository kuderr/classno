...
def cast_bool_from_string(value: str) -> bool:
    ...
    Raises:
        TypeError: If string is not a valid boolean representation
    """
...
def cast_union(value, hint):
    """Cast value to a Union type.
...
def cast_simple_type(value, hint):
    """Cast value to a simple (non-generic) type.
...
def cast_with_origin(value, hint, origin):
    """Cast value to a generic type with origin.
...
def cast_value(value, hint):
    """Main dispatcher for casting values to their target types.
    ...
    if isinstance(hint, types.UnionType) or (origin is not None and origin is t.Union):
        return cast_union(value, hint)

    # Handle simple types (no generic origin)
    if not origin:
        # Value already matches the type
        if isinstance(value, hint):
            return value
        # Need to cast
        return cast_simple_type(value, hint)

    # Handle generic types with origin (List[int], Dict[str, int], etc.)
    return cast_with_origin(value, hint, origin)
